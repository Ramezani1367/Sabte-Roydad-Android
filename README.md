...
## ذخیرهٔ خروجی‌ها (Excel / JSON) روی گوشی

- فایل‌های خروجی به‌صورت پیش‌فرض در **حافظهٔ داخلی › پوشهٔ دانلود › پوشهٔ Backup_Sabteroydad** ذخیره می‌شوند.
- اگر اندروید اجازهٔ نوشتن در دانلود ندهد، اپ خودکار به مسیر «اسناد اپ › Backup_Sabteroydad» می‌رود؛ و در بدترین حالت برگهٔ اشتراک‌گذاری اندروید باز می‌شود تا خودتان محل ذخیره را انتخاب کنید.
- مسیر ذخیره از داخل اپ (☁ بکاپ و انتقال داده › مسیر خروجی فایل‌ها) قابل تغییر است.
- این قابلیت به پلاگین `@capacitor/filesystem` وابسته است که در `package.json` آمده و GitHub Actions خودکار نصب و `cap sync` می‌کند؛ هیچ تنظیم دستی لازم نیست.

## اشتراک‌گذاری واقعی در اندروید

دکمهٔ «📤 اشتراک» در نسخهٔ اندروید از پلاگین بومی `@capacitor/share` استفاده می‌کند و برگهٔ اشتراک‌گذاری واقعی اندروید (تلگرام، واتساپ، ذخیره در فایل‌ها و…) را باز می‌کند. در مرورگرهای معمولی از قابلیت اشتراک خود مرورگر استفاده می‌شود و اگر هیچ‌کدام نبود، متن خودکار کپی می‌شود. این وابستگی هم در `package.json` آمده و GitHub Actions خودکار نصب می‌کند.
